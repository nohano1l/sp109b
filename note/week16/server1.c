...
void connectHandler(int sfd) {
    ...
    for (int i=0; i<CMAX; i++) {
        if (clients[i].fd == 0) {
            ...
            clients[i].fd = cfd;
            pthread_create(&clients[i].thread, NULL, receiver, &i); //創造新phread
            break;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    printf("port=%d\n", port);
    int sfd = socket(AF_INET, SOCK_STREAM, 0); //用tcp(串流)開internet
    struct sockaddr_in saddr, raddr; //宣告server的位置、接收者的位置
    memset(&saddr, 0, sizeof(saddr)); //清零
    saddr.sin_family = AF_INET;
    saddr.sin_port = htons(port);
    char msg[SMAX];
    saddr.sin_addr.s_addr = INADDR_ANY; //所有封包都要收
